...
# Retrieve user ID and provide a message if user is not logged in
try:
...
def recommend_and_create_playlist():
    ...
    num_recommendations = st.number_input("Number of recommendations:", min_value=1, max_value=50, value=20, step=1)
    ...
    try:
        ...
        # Handle any exceptions related to Spotify
        st.error(f"There was an error creating the playlist: {e}")
    # Display the playlist creation message
    st.success(f"Playlist created: {playlist['name']} (ID: {playlist['id']})")
